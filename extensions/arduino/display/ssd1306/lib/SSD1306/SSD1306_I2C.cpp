...
/**
  ...
  */
inline void SSD1306_I2C::setRefreshRange(int address, uint8_t reg, int columnStart, int columnEnd, int rowStart, int rowEnd)
{
...
/**
  ...
  */
inline int SSD1306_I2C::writeBuffer(int address, uint8_t reg, unsigned char *data, int length)
{
...
/**
  ...
  * @return Return the number of data
  */
inline int SSD1306_I2C::writeByte(uint8_t address, uint8_t reg, uint8_t data)
{
    SSD1306_I2CBEGIN();
    Wire.beginTransmission(address);
    Wire.write(reg);
