/*!
 * @file SSD1306.cpp
 * @brief Define the basic structure of class SSD1306
 */
...
/**
  ...
  */
void SSD1306::begin(uint32_t address)
{
...
/**
  ...
  */
void SSD1306::fillScreen(uint32_t color)
{
#if defined(ARDUINO_ESP32_DEV) || defined(NRF5)||defined(NRF52833)
    if(color) {
...
/**
  ...
  * @param color: Set to 0 to show all black, set to 1 to display all white
  */
void SSD1306::fillInLine(uint32_t line, uint32_t color)
{
...
/**
  ...
  * @param color: Set to 0 to show all black, set to 1 to display all white
  */
void SSD1306::setBrushColor(uint32_t color)
{
    brushColor = color;
}
...
/**
  ...
  */
uint32_t SSD1306::getBrushColor()
{
    return brushColor;
}
...
/**
  ...
  */
void SSD1306::setCursor(int32_t x, int32_t y)
{
#if defined(ARDUINO_ESP32_DEV) || defined(NRF5)||defined(NRF52833)
    if(x<0) x=0;
    if(x>127) x=127;
...
/**
  ...
  * @param line: Rows
  */
void SSD1306::setCursorLine(int32_t line)
{
    line = constrain(line, 1, 4);
    cursorLine = line%4;
    if(cursorLine == 0)
...
/**
  ...
  */
void SSD1306::setChCode(const chCode *tp)
{
    tpCode = (chCode *)tp;
}
...
/**
  ...
  */
void SSD1306::setRotaion(eROTATION eRotation)
{
#if defined(ARDUINO_ESP32_DEV) || defined(NRF5)||defined(NRF52833)
    uint8_t buf[16];
	memset(buf, 0, 16);
...
/**
  ...
  */
int SSD1306::getDirection(int value)
{
    if (value >= 0)
        return 1;
    return -1;
...
/**
  ...
  * @return int
  */
int SSD1306::ternaryExpression(bool condition, int o1, int o2)
{
    if(condition)
        return o1;
    return o2;
...
/**
  ...
  */
void SSD1306::showMatrix(uint8_t x, uint8_t y, int width, int height, const uint8_t *pBuf, bool coverage)
{
    if(!pBuf) return;
...
/**
  ...
  */
void SSD1306::print(const char *pCh)
{
#if defined(ARDUINO_ESP32_DEV)
...
/**
  ...
  */
void SSD1306::print(double f, int precision)
{
    String str;
    str = String(f,5);
...
/**
  ...
  * @param pCh: Memory address of the text
  */
void SSD1306::printLine(const char *pCh)
{
...
/**
  ...
  * @param i: uint32_t
  */
void SSD1306::printLine(uint32_t i)
{
    char buffer[34];
    memset(buffer, 0, 34);
    itoa(i, buffer, 10);
...
/**
  ...
  * @param i: int32_t
  */
void SSD1306::printLine(int32_t i)
{
    char buffer[34];
    memset(buffer, 0, 34);
    itoa(i, buffer, 10);
...
/**
  ...
  * @param precision: Number of reserved bits
  */
void SSD1306::printLine(double f, int precision)
{
    String str;
    str = String(f,5);
    char *p = (char *)(str.c_str() + str.length());
...
/**
  ...
  */
void SSD1306::point(int32_t x, int32_t y, uint32_t color)
{
    x = max(min(x, 127), 0);
    y = max(min(y, 63), 0);
...
/**
  ...
  * @param r: radius
  */
void SSD1306::drawCircle(int32_t x0, int32_t y0, int32_t r)
{
    r = abs(r);
...
/**
  ...
  * @param r: radius
  */
void SSD1306::fillCircle(int32_t x0, int32_t y0, int32_t r)
{
    r = abs(r);
    int16_t       varX = 0, varY = r;
    int16_t       endY = 0;
...
/**
    ...
    */
void SSD1306::fillScreenRange(int32_t line, int32_t column1, int32_t column2, uint32_t color)
{
...
/**
  ...
  */
void SSD1306::line(int32_t x0, int32_t y0, int32_t x1, int32_t y1)
{
    x0 = int(x0);
    y0 = int(y0);
    x1 = int(x1);
...
/**
  ...
  */
void SSD1306::rect(int32_t x, int32_t y, int32_t width, int32_t height, bool fill)
{
...
/**
  ...
  */
void SSD1306::drawImage(int32_t x, int32_t y, int32_t width, int32_t height, const uint8_t *image)
{
...
/**
  ...
  */
void SSD1306::lineWidth(uint32_t w)
{
    w = w<1 ? 1 : w;
    brushWidth = w;
}
...
/**
  ...
  * @param height: height
  */
void SSD1306::VLineOnePixel(int16_t x, int16_t y, int16_t height)
{
    int8_t        direction = 1;
...
/**
  ...
  * @param width: width
  */
void SSD1306::HLineOnePixel(int16_t x, int16_t y, int16_t width)
{
    int8_t        direction = 1;
...
/**
  ...
  * @param height: height
  */
void SSD1306::VLine(int16_t x, int16_t y, int16_t height)
{
    x = int(x);
    y = int(y);
    height = int(height);
...
/**
  ...
  * @param width: width
  */
void SSD1306::HLine(int16_t x, int16_t y, int16_t width)
{
    x = int(x);
    y = int(y);
    width = int(width);
...
/**
  ...
  * @param color: color
  */
void SSD1306::writeBufferPixel(int16_t x, int16_t y, uint16_t color)
{
...
/**
  ...
  * @param height: height
  */
void SSD1306::regionalRefresh(int16_t x, int16_t y, int16_t width, int16_t height)
{
