...
enum protocolCommand
{
    ...
    COMMAND_REQUEST_LEARN_ONCE = 0x36,
...
public:
    ...
    bool learnOnce(uint16_t id)
    {
        uint8_t data[] = {id & 0xff, (id >> 8) & 0xff};
        Protocol_t protocol;
        protocol.length = 2;
        protocol.data = data;
        protocolWriteRequestlearnOnce(protocol);
        return wait(COMMAND_RETURN_OK);
    }
...
#define PROTOCOL_CREATE(function, type, command)       \
    ...
    PROTOCOL_CREATE(RequestlearnOnce, BufferUint8, COMMAND_REQUEST_LEARN_ONCE)
